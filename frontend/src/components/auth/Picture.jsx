...
const Picture = ({ setPicture, setReadablePicture, readablePicture }) => {
  //   const { error } = useSelector((state) => state.user);
  const [error, setError] = useState("");
  const inputRef = useRef();
  const hanldePicture = (e) => {
    console.log("i am logginf event of handlepicture firstHnd",e);
    ...
    if (
      ...
      //5mb
      setError(`${pic.name} size exeeds 5mb`);
      return;
    } else {
      setPicture(pic);
      //reading the picture
      ...
      reader.onload = (e) => {
        console.log("i am logging the event i am getting from reader.onload");
